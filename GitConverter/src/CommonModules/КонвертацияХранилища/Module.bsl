...
Процедура ВыполнитьКонвертацию(Знач Хранилище = Неопределено) Экспорт
	...
	Если НЕ Параметры.КонвертироватьВФорматEDT Тогда
		ВызватьИсключение НСтр("ru = 'Конветрация в формат xml 1С:Предприятия не поддерживается.'");
	КонецЕсли;
...
Процедура ВыполнитьКоммиты(Знач Хранилище) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если РеквизитыХранилища.КонвертацияВВеткуСуществующегоРепозитория Тогда
			// В случае, если комиты делаем не в пустое хранилище файл проекта не замещаем, и предварительно удаляем из исходной папки 
			ИсточникДляПроекта = СокрЛП(ПараметрыВерсии.ПутьКПроектуВерсии);
			Если Прав(ИсточникДляПроекта, 1) = ПолучитьРазделительПути() Тогда
				ИсточникДляПроекта = Лев(ИсточникДляПроекта, СтрДлина(ИсточникДляПроекта) - 1);
			КонецЕсли;
			ИмяФайлаПроекта = ИсточникДляПроекта+ПолучитьРазделительПутиСервера()+".project"; 
			ФайлПроекта = Новый Файл(ИмяФайлаПроекта);
			Если ФайлПроекта.Существует() Тогда
				УдалитьФайлы(ИмяФайлаПроекта);
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьПараметрыОбработкиВерсии(Знач ВерсияХранилища) Экспорт
	...
	КонецЕсли;
	Параметры.Вставить("Это8315", ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Параметры.ВерсияПлатформы, "8.3.15.0") > 1);
...
// Параметры:
// 	Хранилище - СправочникСсылка.ХранилищаКонфигураций - Ссылка на хранилище, по которому выполняются коммиты
Процедура КлонироватьРепозиторийGit(Хранилище) Экспорт
	...
	ФайлЛога = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(РеквизитыХранилища.КаталогВыгрузкиВерсий)
		+ "git_log_clone" + ".txt";
	ФайлКомандыGit = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(РеквизитыХранилища.КаталогВыгрузкиВерсий)
		+ "git_command_clone" + ?(ЭтоWindowsСервер, ".bat", ".sh");

	МассивКаталогов = СтрРазделить(РеквизитыХранилища.КаталогВыгрузкиВерсий, ПолучитьРазделительПути(), Ложь);
	ПутьККаталогу = ""; 
	...
	// Отключаем верификацию SSL
	ТекстКоманды = "git config --global http.%АдресРепозиторияGit%.sslverify false" + ВыводЛога;
	СсылкаСервера = РеквизитыХранилища.АдресРепозиторияGit;
	ПозицияПоследнейТочкиАдреса = СтрНайти(СсылкаСервера, ".", НаправлениеПоиска.СКонца);
	ПозицияСлешаПослеАдреса = СтрНайти(СсылкаСервера, "/", НаправлениеПоиска.СНачала, ПозицияПоследнейТочкиАдреса);
	СсылкаСервера = Лев(СсылкаСервера,ПозицияСлешаПослеАдреса-1);
	ТекстКоманды = СтрЗаменить(ТекстКоманды, "%АдресРепозиторияGit%", СсылкаСервера);
	ФайлКоманды.ДобавитьСтроку(ТекстКоманды);

	// Клонируем ветку репозитория с глубиной 2 
	ТекстКомандыКлонированияКаталога = "git clone --depth 2 -b ""%ИмяВетки%"" ""%АдресРепозиторияGit%"" ""%ЛокальныйКаталогGit%""" + ВыводЛога;
	ТекстКомандыКлонированияКаталога = СтрЗаменить(ТекстКомандыКлонированияКаталога, "%ЛокальныйКаталогGit%", РеквизитыХранилища.ЛокальныйКаталогGit);
	ТекстКомандыКлонированияКаталога = СтрЗаменить(ТекстКомандыКлонированияКаталога, "%ИмяВетки%", РеквизитыХранилища.ИмяВетки);
	АдресРепозиторияGit = РеквизитыХранилища.АдресРепозиторияGit;
	ПозицияРазделителя = СтрНайти(АдресРепозиторияGit, "://");
	Если ПозицияРазделителя > 0 Тогда
		АдресРепозиторияGit = Лев(АдресРепозиторияGit, ПозицияРазделителя + 2)
			...
			- ПозицияРазделителя - 2);
	КонецЕсли;
	ТекстКомандыКлонированияКаталога = СтрЗаменить(ТекстКомандыКлонированияКаталога, "%АдресРепозиторияGit%", АдресРепозиторияGit);
	

	ФайлКоманды.ДобавитьСтроку(ТекстКомандыКлонированияКаталога);
	...
	// Отключаем верификацию SSL
	ТекстКоманды = "git config --local http.%АдресРепозиторияGit%.sslverify false" + ВыводЛога;
	СсылкаСервера = РеквизитыХранилища.АдресРепозиторияGit;
	ПозицияПоследнейТочкиАдреса = СтрНайти(СсылкаСервера, ".", НаправлениеПоиска.СКонца);
	ПозицияСлешаПослеАдреса = СтрНайти(СсылкаСервера, "/", НаправлениеПоиска.СНачала, ПозицияПоследнейТочкиАдреса);
	СсылкаСервера = Лев(СсылкаСервера,ПозицияСлешаПослеАдреса-1);
...
Процедура ЗаписатьФайлыКоммитаВерсии(ВерсияХранилища, ПредыдущаяВерсия, РеквизитыХранилища, РеквизитыВерсии, 
	...
	Если РеквизитыХранилища.КонвертацияВВеткуСуществующегоРепозитория Тогда
		// В случае, если комиты делаем не в пустое хранилище файл проекта не замещаем, и предварительно удаляем из исходной папки 
		ИсточникДляПроекта = СокрЛП(РеквизитыВерсии.ПутьКПроектуВерсии);
		Если Прав(ИсточникДляПроекта, 1) = ПолучитьРазделительПути() Тогда
			ИсточникДляПроекта = Лев(ИсточникДляПроекта, СтрДлина(ИсточникДляПроекта) - 1);
		КонецЕсли;
		ИмяФайлаПроекта = ИсточникДляПроекта+ПолучитьРазделительПутиСервера()+".project"; 
...
Функция ПараметрыХранилищаДляКоммита(Знач Хранилище)
	...
	ИменаРеквизитов = Новый Структура("ВерсияВGit, Код, ЛокальныйКаталогGit, КаталогВыгрузкиВерсий, АдресРепозиторияGit, 
		...
		| КонвертироватьВФорматEDT, ДобавлятьМеткиСВерсиейКонфигурации, КонвертацияВВеткуСуществующегоРепозитория", 
		"ВерсияВGit", 
		"ВерсияВGit.Код");
	РеквизитыХранилища = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Хранилище, ИменаРеквизитов);
