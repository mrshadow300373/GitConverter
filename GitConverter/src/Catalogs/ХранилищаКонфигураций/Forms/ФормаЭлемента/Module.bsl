...
&НаКлиенте
Процедура НеЗамещатьФайлПроектаПриИзменении(Элемент)
...
&НаКлиенте
Процедура КлонироватьВетку(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ЛокальныйКаталогGit) Тогда
		...
		ТекстПредупреждения = НСтр("ru='Не указан локальный каталог репозитория Git'");
		Отказ = Истина;
	...
	Если НЕ ЗначениеЗаполнено(Объект.АдресРепозиторияGit) Тогда
		...
		ТекстПредупреждения = ?(ТекстПредупреждения = "","",Символы.ПС) + НСтр("ru='Не указан адрес репозитория Git'");
		Отказ = Истина;
	
	КонецЕсли; 
	
	Если НЕ ЗначениеЗаполнено(Объект.ИмяВетки) Тогда
		
		ТекстПредупреждения = ?(ТекстПредупреждения = "","",Символы.ПС) + НСтр("ru='Не указано имя ветки'");
		Отказ = Истина;
	...
	КлонироватьРепозиторийGitНаСервере();
...
Функция ПолучитьКаталогиПроектовВРепозиторииНаСервере(Знач ЛокальныйКаталогGit)
	
	ЛокальныйКаталогGit = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ЛокальныйКаталогGit);
	
	Если ЗначениеЗаполнено(ЛокальныйКаталогGit) Тогда
		МассивФайлов = НайтиФайлы(ЛокальныйКаталогGit, "src", Истина);
		МассивКаталоговДляВыбора = Новый Массив;
		Для Каждого Файл Из МассивФайлов Цикл
			ОтносительныйПутьСоСлешем = Сред(Файл.Путь, СтрДлина(ЛокальныйКаталогGit) + 1);
			МассивКаталоговДляВыбора.Добавить(СтрЗаменить(Лев(ОтносительныйПутьСоСлешем, СтрДлина(ОтносительныйПутьСоСлешем) - 1),"\","/"));
		КонецЦикла;
	КонецЕсли;
	
	Возврат МассивКаталоговДляВыбора;
...
Процедура УстановитьДоступность(Форма)
	...
	КонвертацияВВеткуСуществующегоРепозитория = Форма.Объект.КонвертацияВВеткуСуществующегоРепозитория; 
	
	Форма.Элементы.СоздатьРепозиторийGit.Видимость = НЕ КонвертацияВВеткуСуществующегоРепозитория;
	Форма.Элементы.КлонироватьВетку.Видимость = КонвертацияВВеткуСуществующегоРепозитория;
	
	Форма.Элементы.ИмяПроектаEDT.КнопкаВыпадающегоСписка = КонвертацияВВеткуСуществующегоРепозитория;
	Форма.Элементы.ИмяПроектаEDT.РедактированиеТекста = НЕ КонвертацияВВеткуСуществующегоРепозитория;
	Если КонвертацияВВеткуСуществующегоРепозитория И НЕ Форма.Элементы.СоздатьРепозиторийGit.Доступность Тогда
		МассивКаталогов = ПолучитьКаталогиПроектовВРепозиторииНаСервере(Форма.Объект.ЛокальныйКаталогGit);
		Форма.Элементы.ИмяПроектаEDT.СписокВыбора.ЗагрузитьЗначения(МассивКаталогов);
	КонецЕсли;
...
Процедура ПроверитьНаличиеРепозитория()
	...
	Для каждого Файл Из НайденныеФайлы Цикл
		...
		Если НРег(Файл.Имя) = ".git" Тогда
			...
			Элементы.СоздатьРепозиторийGit.Доступность = Ложь;
			Элементы.ФормаСоздатьРепозиторийGit.Доступность = Ложь;
			Элементы.КлонироватьВетку.Доступность = Ложь;
			Если Объект.КонвертацияВВеткуСуществующегоРепозитория Тогда
				Элементы.ИмяПроектаEDT.СписокВыбора.ЗагрузитьЗначения(ПолучитьКаталогиПроектовВРепозиторииНаСервере(Объект.ЛокальныйКаталогGit));
			КонецЕсли;
			Возврат;
...
&НаСервере
Процедура КлонироватьРепозиторийGitНаСервере()
	
	КонвертацияХранилища.КлонироватьРепозиторийGit(Объект.Ссылка);
